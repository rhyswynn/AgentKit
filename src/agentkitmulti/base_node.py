...
class BaseNode:
    ...
    Attributes:
        ...
        query_function (Callable): Function to perform node action.
    ...
    def _query_function(self, prompt, shrink_idx):

        start_time_ms = round(datetime.datetime.now().timestamp() * 1000)
        result = self.query_function(prompt, shrink_idx)
        ...
        if not self.temporary_skip:
            ...
            for i in range(3):
                try:
                    ...
                    self.result, usage = self._query_function(temp_prompt, shrink_idx)
